...
func TestCodexSwitchSharedConfigWritesOnlyAuthAndCapturesRefresh(t *testing.T) {
	...
	if _, err := ApplySwitch(ctx, ApplySwitchRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "first", Confirm: true}); err != nil {
	...
	if _, err := ApplySwitch(ctx, ApplySwitchRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "second", Confirm: true}); err != nil {
...
func TestCodexSwitchSharedCredentialWritesOnlyConfig(t *testing.T) {
	...
	if _, err := CreateCodexProfile(ctx, CreateCodexProfileRequest{ConfigDir: configDir, CodexDir: codexDir, ProfileID: "first"}); err != nil {
		t.Fatalf("expected first profile, got %v", err)
	}
	if _, err := ForkCodexProfile(ctx, ForkCodexProfileRequest{
		ConfigDir: configDir, CodexDir: codexDir, SourceProfileID: "first", ProfileID: "second",
	...
	plan, err := BuildPlan(ctx, BuildPlanRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "second"})
	if err != nil {
...
func TestCodexSwitchCapturesBothChangedWorkingCopies(t *testing.T) {
	...
	updatedPlan, err := BuildPlan(ctx, BuildPlanRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "first"})
	if err != nil {
	...
	plan = updatedPlan
	if _, err := ApplySwitch(ctx, ApplySwitchRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "first", ExpectedPlanFingerprint: plan.PlanFingerprint, Confirm: true}); err != nil {
		t.Fatalf("expected switch to first, got %v", err)
	}
	if _, err := ApplySwitch(ctx, ApplySwitchRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "second", Confirm: true}); err != nil {
		t.Fatalf("expected switch back to second, got %v", err)
...
func TestCodexRollbackRestoresActiveFilesWithoutUndoingCapturedState(t *testing.T) {
	...
	switchResult, err := ApplySwitch(ctx, ApplySwitchRequest{ConfigDir: configDir, ProviderID: codexconfig.ProviderID, ProfileID: "first", Confirm: true})
	if err != nil {
		t.Fatalf("expected switch, got %v", err)
	}
	if _, err := ApplyRollback(ctx, ApplyRollbackRequest{ConfigDir: configDir, BackupID: switchResult.OperationID, Confirm: true}); err != nil {
...
func setupCodexSwitchProfiles(t *testing.T, independentConfig bool) (context.Context, string, string) {
	...
	request := CreateCodexProfileRequest{ConfigDir: configDir, CodexDir: codexDir, ProfileID: "second"}
	if independentConfig {
		request.NewConfigSetID = "second-config"
	}
	if _, err := CreateCodexProfile(ctx, request); err != nil {
