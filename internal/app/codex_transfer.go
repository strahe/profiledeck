...
func ExportCodexProfiles(ctx context.Context, req ExportCodexProfilesRequest) (CodexProfileExportResult, error) {
	outputPath, appErr := normalizeTransferPath(req.OutputPath, ErrorExportFailed, "output path")
	if appErr != nil {
	...
	if err != nil {
		return CodexProfileExportResult{}, WrapError(ErrorExportFailed, "failed to build Codex profile bundle", err)
	}
	if appErr := rejectUnsafeCodexExportPath(ctx, db, req.ConfigDir, outputPath); appErr != nil {
		return CodexProfileExportResult{}, appErr
	}
	raw, err := profilebundle.Encode(bundle)
	if err != nil {
		return CodexProfileExportResult{}, WrapError(ErrorExportFailed, "failed to encode Codex profile bundle", err)
	}
	...
	if err != nil {
		return CodexProfileExportResult{}, codexTransferFileError(ErrorExportFailed, "failed to write Codex profile bundle", outputPath, err)
	}
...
func InspectCodexProfileImport(ctx context.Context, req InspectCodexProfileImportRequest) (CodexProfileImportPlan, error) {
	...
	if err != nil {
		return CodexProfileImportPlan{}, WrapError(ErrorStoreStatusFailed, "failed to inspect Codex profile import state", err)
	}
	return plan, nil
}

func ImportCodexProfiles(ctx context.Context, req ImportCodexProfilesRequest) (CodexProfileImportResult, error) {
	if !req.Confirm {
		return CodexProfileImportResult{}, NewError(ErrorConfirmationRequired, "Codex profile import apply requires confirmation")
	}
	expectedFingerprint := strings.TrimSpace(req.ExpectedPlanFingerprint)
	if expectedFingerprint == "" {
		return CodexProfileImportResult{}, NewError(ErrorConfirmationRequired, "Codex profile import apply requires a reviewed plan fingerprint")
	}
	home, err := resolveCodexMutationHome(req.CodexDir)
	if err != nil {
	...
	db, lock, operationID, err := openLockedCodexStore(ctx, req.ConfigDir, "profile-import")
	if err != nil {
		return CodexProfileImportResult{}, err
	}
	defer db.Close()
	...
	err = db.WithTransaction(ctx, func(txStore *store.Store) error {
		...
		if plan.PlanFingerprint != expectedFingerprint {
			return NewError(ErrorImportPlanChanged, "Codex profile import plan changed after preview").
		...
		if plan.Counts.Conflict > 0 {
			return NewError(ErrorImportConflict, "Codex profile import contains conflicts").WithDetail("conflict_count", plan.Counts.Conflict)
		}
		...
		if _, err := txStore.CreateAppliedImportOperation(ctx, store.CreateAppliedImportOperationParams{ID: operationID, MetadataJSON: metadata}); err != nil {
			return WrapError(ErrorOperationCreateFailed, "failed to record Codex profile import", err)
		}
		changed = true
		return nil
	})
...
func buildCodexProfileBundle(ctx context.Context, db *store.Store, profileIDs []string) (profilebundle.Bundle, error) {
	...
	for _, profileID := range profileIDs {
		...
		configTarget, authTarget, err := requireCodexFullProfileTargets(profileID, profileTargets)
		if err != nil || len(profileTargets) != 2 {
			return profilebundle.Bundle{}, NewError(ErrorCodexInvalid, "Codex profile is not exportable as a full profile").WithDetail("profile_id", profileID)
		}
...
func readCodexProfileBundle(ctx context.Context, rawPath string) (decodedCodexProfileBundle, error) {
	path, appErr := normalizeTransferPath(rawPath, ErrorImportInvalid, "input path")
	if appErr != nil {
		return decodedCodexProfileBundle{}, appErr
	}
	read, err := transferfile.ReadPrivate(ctx, path, profilebundle.MaxBundleBytes)
	if err != nil {
		return decodedCodexProfileBundle{}, codexTransferFileError(ErrorImportInvalid, "failed to read Codex profile bundle", path, err)
	}
	bundle, err := profilebundle.Decode(read.Content)
	if err != nil {
		return decodedCodexProfileBundle{}, WrapError(ErrorImportInvalid, "Codex profile bundle is invalid", err).WithDetail("path", path)
	}
...
func planCodexProviderImport(ctx context.Context, db *store.Store, home codexconfig.Home) CodexProfileImportItem {
	...
	if item.Action != CodexProfileImportActionConflict {
		if ensureProfileTargetPathOwnership(ctx, db, home.ConfigPath, targetPathOwnershipKey(home.ConfigPath), codexconfig.ProviderID, codexconfig.TargetID, nil) != nil ||
			ensureProfileTargetPathOwnership(ctx, db, home.AuthPath, targetPathOwnershipKey(home.AuthPath), codexconfig.ProviderID, codexconfig.AuthTargetID, nil) != nil {
...
func normalizeOptionalProfileIDs(values []string) ([]string, *AppError) {
	seen := make(map[string]struct{}, len(values))
	result := make([]string, 0, len(values))
	for _, raw := range values {
		id, appErr := validateID(raw, ErrorProfileInvalid)
		if appErr != nil {
			return nil, appErr
		}
...
func normalizeTransferPath(raw string, code ErrorCode, label string) (string, *AppError) {
	value := strings.TrimSpace(raw)
	if value == "" {
		return "", NewError(code, label+" is required")
	}
	path, err := filepath.Abs(value)
	if err != nil {
		return "", WrapError(code, "failed to resolve "+label, err)
	}
...
func rejectUnsafeCodexExportPath(ctx context.Context, db *store.Store, configDir, outputPath string) *AppError {
	...
	if err != nil {
		...
		return WrapError(ErrorExportFailed, "failed to resolve ProfileDeck runtime paths", err)
	}
