...
func TestClaudeCodeKeychainBackendPassiveReadDoesNotRequestAuthenticationUI(t *testing.T) {
	...
	backend := claudeCodeKeychainTargetBackend{driver: driver}
	spec := claudeCodeKeychainTargetSpec{ID: "auth", Service: "Claude Code-credentials", Account: "tester"}
	if _, err := backend.InspectWithInteraction(context.Background(), spec, false); !isClaudeCodeKeychainAuthorizationRequired(err) {
...
func TestClaudeCodeKeychainBackendRequiresOneMatchingReference(t *testing.T) {
	spec := claudeCodeKeychainTargetSpec{ID: "auth", Service: "Claude Code-credentials", Account: "tester"}
	backend := claudeCodeKeychainTargetBackend{driver: &fakeClaudeCodeKeychainDriver{items: map[string]claudekeychain.Item{}}}
	snapshot, err := backend.Inspect(context.Background(), spec)
	if err != nil || snapshot.Exists {
		t.Fatalf("zero-reference Inspect() = %#v, error = %v", snapshot, err)
	}
	backend.driver = &fakeClaudeCodeKeychainDriver{
		references: []claudekeychain.Reference{
	...
	if _, err := backend.Inspect(context.Background(), spec); !isAppErrorCode(err, ErrorClaudeCodeInvalid) {
	...
	if _, err := backend.Inspect(context.Background(), spec); !isAppErrorCode(err, ErrorTargetChanged) {
...
func TestClaudeCodeKeychainBackendReadsRawDataAndUpdatesExactReference(t *testing.T) {
	...
	backend := claudeCodeKeychainTargetBackend{driver: driver}
	spec := claudeCodeKeychainTargetSpec{ID: "auth", Service: "Claude Code-credentials", Account: "tester", Label: "Claude Code login"}
	snapshot, err := backend.Inspect(context.Background(), spec)
	if err != nil {
...
func TestClaudeCodeKeychainBackendRejectsAmbiguousReplacementAndChangedContent(t *testing.T) {
	...
	if err := backend.Verify(context.Background(), spec, snapshot); !isAppErrorCode(err, ErrorTargetChanged) {
	...
	driver.items[string(reference)] = item
	if err := backend.Apply(context.Background(), spec, snapshot, "desired", 0, false); !isAppErrorCode(err, ErrorTargetChanged) {
...
func TestClaudeCodeKeychainBackendRejectsDeletedReferenceAndPostVerifyFailure(t *testing.T) {
	...
	if err := backend.Apply(context.Background(), spec, snapshot, "desired", 0, false); !isAppErrorCode(err, ErrorTargetWriteFailed) {
...
func TestClaudeCodeKeychainBackupKeepsReferenceOutOfPublicSummary(t *testing.T) {
	...
	if err != nil || hash != sha256HexString("secret") {
	...
	if _, err := backend.Backup(context.Background(), spec, snapshot, destination); !isAppErrorCode(err, ErrorBackupFailed) {
		t.Fatalf("Backup() overwrote an existing destination: %v", err)
	}
	public := BackupEntrySummary{TargetID: "auth", BackendID: targetBackendClaudeCodeKeychain}
