...
func TestRecoverFailedSwitchRemovesPartiallyCreatedTargets(t *testing.T) {
	...
	result, err := RecoverFailedSwitch(ctx, RecoverFailedSwitchParams{
		ConfigDir:   configDir,
...
func TestRecoverFailedSwitchRejectsUnrecoverableCheckpoint(t *testing.T) {
	...
	_, err = RecoverFailedSwitch(ctx, RecoverFailedSwitchParams{
		ConfigDir:   configDir,
		OperationID: "switch-before-backup",
		Confirm:     true,
	})
	assertAppErrorCode(t, err, ErrorRecoveryUnsupported)
