...
func UsageReport(ctx context.Context, req UsageReportRequest) (UsageReportResult, error) {
	return usageReportAt(ctx, req, time.Now())
}

func usageReportAt(ctx context.Context, req UsageReportRequest, now time.Time) (UsageReportResult, error) {
	...
	preset, err := usage.ParseRangePreset(string(rangeValue))
	if err != nil {
		return UsageReportResult{}, NewError(ErrorUsageInvalid, "unsupported usage range")
	}
	...
	var resolved usage.ResolvedRange
	...
	err = db.WithTransaction(ctx, func(txStore *store.Store) error {
		...
		resolved, err = usage.ResolveRange(preset, now, earliest)
		if err != nil {
			return err
		}
	...
	if err != nil {
		return UsageReportResult{}, WrapError(ErrorStoreStatusFailed, "failed to read usage report", err)
	}
	...
	result := UsageReportResult{
		...
		Pricing: UsagePricingInfo{
			Basis:               usage.PricingBasis,
			SourceURL:           usage.PricingSourceURL,
			VerifiedAt:          usage.PricingVerifiedAt,
...
func usageAggregateSummary(aggregate store.UsageAggregate) UsageAggregateSummary {
	...
	return UsageAggregateSummary{
		...
		KnownEstimatedCostUSD:   usage.USDStringFromMicros(aggregate.EstimatedCostMicros),
