...
func TestUsageReportRangesUndatedAndPartialPricing(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			report, reportErr := usageReportAt(ctx, UsageReportRequest{ConfigDir: configDir, ProviderID: "codex", Range: test.rangeValue}, now)
			if reportErr != nil {
	...
	week, err := usageReportAt(ctx, UsageReportRequest{ConfigDir: configDir, Range: UsageRange7Days}, now)
	if err != nil {
	...
	all, err := usageReportAt(ctx, UsageReportRequest{ConfigDir: configDir, Range: UsageRangeAll}, now)
	if err != nil {
...
func TestUsageReportTodayUsesDSTBoundaries(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			report, reportErr := usageReportAt(ctx, UsageReportRequest{ConfigDir: configDir, Range: UsageRangeToday}, test.now)
			if reportErr != nil {
...
func TestUsageReportEmptyDefaultsAndValidation(t *testing.T) {
	...
	report, err := usageReportAt(ctx, UsageReportRequest{ConfigDir: configDir}, time.Date(2026, time.July, 10, 12, 0, 0, 0, time.UTC))
	if err != nil {
	...
	_, err = UsageReport(ctx, UsageReportRequest{ConfigDir: configDir, Range: "14d"})
	assertAppErrorCode(t, err, ErrorUsageInvalid)
	_, err = UsageReport(ctx, UsageReportRequest{ConfigDir: configDir, ProviderID: "claude"})
	assertAppErrorCode(t, err, ErrorUsageInvalid)
}
