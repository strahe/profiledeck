...
// DesiredContent applies the target strategy to an inspected target snapshot.
func DesiredContent(target Target, before Read) (string, []string, error) {
	switch target.Strategy {
	case StrategyReplaceFile:
		content, err := ReplaceFileContent(target.ValueJSON)
	...
	case StrategyEnvMerge:
		...
		return content, nil, err
	default:
		return "", nil, apperror.New(apperror.TargetInvalid, "target strategy is invalid")
	}
}

// ReplaceFileContent extracts a validated replacement value from value_json.
func ReplaceFileContent(raw string) (string, error) {
	value, appErr := DecodeSingleJSONObject(raw, apperror.TargetInvalid, "stored value_json")
	if appErr != nil {
		return "", appErr
	}
	content, ok := value["content"].(string)
	if !ok || len(value) != 1 {
		return "", apperror.New(apperror.TargetInvalid, `stored replace-file value_json must be {"content": string}`)
	}
	return content, nil
}

func mergeJSONTargetContent(target Target, before Read) (string, error) {
	patch, appErr := DecodeSingleJSONObject(target.ValueJSON, apperror.TargetInvalid, "stored value_json")
	if appErr != nil {
		return "", appErr
	}
...
func decodeJSONTargetObject(raw string, target Target) (map[string]any, *apperror.Error) {
...
func mergeTOMLTargetContent(target Target, before Read) (string, error) {
	patch, appErr := DecodeSingleJSONObject(target.ValueJSON, apperror.TargetInvalid, "stored value_json")
	if appErr != nil {
		return "", appErr
	}
...
func jsonObjectToTOML(value map[string]any, target Target) (map[string]any, *apperror.Error) {
...
func jsonValueToTOML(value any, target Target) (any, *apperror.Error) {
...
func mergeEnvTargetContent(target Target, before Read) (string, error) {
	patch, appErr := DecodeSingleJSONObject(target.ValueJSON, apperror.TargetInvalid, "stored value_json")
	if appErr != nil {
		return "", appErr
	}
...
func validateStoredTargetValue(target Target, value map[string]any) *apperror.Error {
	appErr := ValidateValueShape(target.Format, target.Strategy, value)
	if appErr == nil {
		return nil
	}
	return appErr.WithDetail("path", target.Path).WithDetail("format", target.Format).WithDetail("strategy", target.Strategy)
}

func envPatchValues(patch map[string]any, target Target) (map[string]string, *apperror.Error) {
...
func mergeEnvContent(content string, patch map[string]string, target Target) (string, error) {
...
func parseEnvLine(line envLine, target Target) (envLine, *apperror.Error) {
...
func envValueEnd(body string, start int, target Target, key string) (int, *apperror.Error) {
...
func envCommentSuffixStart(body string, start int, target Target, key string) (int, *apperror.Error) {
	quote := byte(0)
	escaped := false
...
func isEnvHorizontalSpace(value byte) bool { return value == ' ' || value == '\t' }
func isEnvNameStart(value byte) bool {
	return value == '_' || value >= 'A' && value <= 'Z' || value >= 'a' && value <= 'z'
}
func isEnvNamePart(value byte) bool { return isEnvNameStart(value) || value >= '0' && value <= '9' }
...
func targetContentInvalidError(target Target, message string, cause error) *apperror.Error {
	var appErr *apperror.Error
	if cause == nil {
		appErr = apperror.New(apperror.TargetInvalid, message)
	} else {
		appErr = apperror.Wrap(apperror.TargetInvalid, message, cause)
	}
	return appErr.WithDetail("path", target.Path).WithDetail("format", target.Format).WithDetail("strategy", target.Strategy)
}
