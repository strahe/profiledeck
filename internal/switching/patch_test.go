...
const (
	targetFormatJSON           = profiletarget.FormatJSON
	targetFormatTOML           = profiletarget.FormatTOML
	targetFormatEnv            = profiletarget.FormatEnv
	targetStrategyJSONMerge    = profiletarget.StrategyJSONMerge
	targetStrategyTOMLMerge    = profiletarget.StrategyTOMLMerge
	targetStrategyEnvMerge     = profiletarget.StrategyEnvMerge
	tomlSemanticRewriteWarning = profiletarget.TOMLSemanticRewriteWarning
)
...
func buildTestPatchOperation(t *testing.T, format, strategy, valueJSON string, before *string) switchplan.Operation {
	t.Helper()
	...
	spec := switchtarget.FileSpec{ID: stored.TargetID, Path: stored.Path, NeedsContent: true, Label: stored.Path}
	snapshot, err := (switchtarget.FileBackend{}).Inspect(context.Background(), spec)
	if err != nil {
		t.Fatalf("expected target inspection to succeed, got %v", err)
	}
	op, err := switchplan.BuildGenericOperation(
		switchplan.Provider{ID: "provider-a", AdapterID: "generic"},
		switchplan.Profile{ID: "profile-a"},
		stored,
		spec,
		snapshot,
	)
...
func desiredTargetContent(target store.ProfileTarget, before targetPlanRead) (string, []string, error) {
	return profiletarget.DesiredContent(profiletarget.Target{
		Path: target.Path, Format: target.Format, Strategy: target.Strategy, ValueJSON: target.ValueJSON,
	}, profiletarget.Read{Exists: before.FileExists, Content: before.Content})
}
