...
func TestCodexDetectReportsPresetHomeConflict(t *testing.T) {
	...
	if _, err := newCodexTestEnvironment(t, configDir, codexDirA).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "work",
	}); err != nil {
		t.Fatalf("expected Codex profile create to succeed, got %v", err)
	}

	result, err := newCodexTestEnvironment(t, configDir, codexDirB).codex.Detect(ctx)
	if err != nil {
...
func TestCodexPlanIgnoresGenericTargetPath(t *testing.T) {
	...
	if _, err := db.CreateProfileTarget(ctx, store.CreateProfileTargetParams{
		...
		Path: mutatedPath, PathKey: profiletarget.PathOwnershipKey(mutatedPath), Format: profiletarget.FormatTOML,
		Strategy: profiletarget.StrategyReplaceFile, ValueJSON: `{}`, Enabled: true, MetadataJSON: `{}`,
	}); err != nil {
		_ = db.Close()
...
func TestCodexPlanRejectsUnsupportedTypedSlot(t *testing.T) {
	...
	_, err = newCodexTestEnvironment(t, configDir, "").switching.BuildPlan(ctx, switching.BuildPlanRequest{
...
func TestCodexProfileCreateCanAttachToExistingGlobalProfile(t *testing.T) {
	...
	if _, err := newCodexTestEnvironment(t, configDir, "").profiles.Create(ctx, profile.CreateRequest{
		ID: "shared", Name: "Shared Profile", Description: "Used by multiple Agents",
	}); err != nil {
	...
	result, err := newCodexTestEnvironment(t, configDir, codexDir).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "shared",
	})
	if err != nil {
...
func TestCodexProfileCreateRejectsConflictingProviderAndHome(t *testing.T) {
	...
	writeCodexProfileFixture(t, codexDirA, `model = "gpt-5-codex"`+"\n", `{"tokens":{"account_id":"remote-work","access_token":"secret"}}`)
	_, err = newCodexTestEnvironment(t, configDir, codexDirA).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "work",
	})
	assertErrorCode(t, err, apperror.CodexInvalid)
	...
	_, err = newCodexTestEnvironment(t, configDir, codexDirB).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "personal",
	})
	assertErrorCode(t, err, apperror.CodexInvalid)
}
...
func TestCodexGenericManagedTargetIsNotAProfileBinding(t *testing.T) {
	...
	if _, err := db.CreateProfileTarget(ctx, store.CreateProfileTargetParams{
		...
		PathKey:      profiletarget.PathOwnershipKey(home.ConfigPath),
		Format:       profiletarget.FormatTOML,
		Strategy:     profiletarget.StrategyTOMLMerge,
	...
	list, err := newCodexTestEnvironment(t, configDir, "").codex.ListProfiles(ctx)
	if err != nil {
...
func TestCodexGenericAccountRefTargetIsNotAProfileBinding(t *testing.T) {
	...
	if _, err := db.CreateProfileTarget(ctx, store.CreateProfileTargetParams{
		...
		PathKey:      profiletarget.PathOwnershipKey(home.ConfigPath),
		Format:       profiletarget.FormatTOML,
		Strategy:     profiletarget.StrategyReplaceFile,
	...
	if _, err := db.CreateProfileTarget(ctx, store.CreateProfileTargetParams{
		...
		Path:         home.AuthPath,
		PathKey:      profiletarget.PathOwnershipKey(home.AuthPath),
		Format:       profiletarget.FormatJSON,
		Strategy:     profiletarget.StrategyReplaceFile,
...
func TestCodexProfileCreateSwitchesFullConfigAndAuthThroughPipeline(t *testing.T) {
	...
	created, err := newCodexTestEnvironment(t, configDir, codexDir).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "work",
		Name:      stringPointer("Work"),
	})
	if err != nil {
		t.Fatalf("expected Codex profile create to succeed, got %v", err)
	}
...
func TestCodexProfileCreateHandlesMissingFiles(t *testing.T) {
	...
	if _, err := initCodexTestRuntime(ctx, emptyConfigDir); err != nil {
		t.Fatalf("expected second init to succeed, got %v", err)
	}
	...
	emptyResult, err := newCodexTestEnvironment(t, emptyConfigDir, emptyCodexDir).codex.CreateProfile(ctx, CreateCodexProfileRequest{
		ProfileID: "work",
	})
	if err != nil {
...
func hasOperationPreview(operations []switching.PlanOperation, targetID, preview string) bool {
...
func planPreviewText(operations []switching.PlanOperation) string {
