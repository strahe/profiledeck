...
func (service *Service) CreateProfile(ctx context.Context, req CreateCodexProfileRequest) (CodexProfileSaveResult, error) {
	...
	err = service.maintenance.RunMaintenance(ctx, maintenance.Request{
		...
		if err != nil {
			return apperror.Wrap(apperror.CodexInvalid, "failed to generate Codex credential id", err)
		}
...
func (service *Service) ForkProfile(ctx context.Context, req ForkCodexProfileRequest) (CodexProfileSaveResult, error) {
	if err := service.requireAccess(ctx); err != nil {
		return CodexProfileSaveResult{}, err
	}
	sourceProfileID, appErr := validateID(req.SourceProfileID, apperror.ProfileInvalid)
	if appErr != nil {
		return CodexProfileSaveResult{}, appErr
	}
	profileID, appErr := validateID(req.ProfileID, apperror.ProfileInvalid)
	...
	if credentialBinding == CodexForkBindingShareParent && configBinding == CodexForkBindingShareParent {
		return CodexProfileSaveResult{}, apperror.New(apperror.CodexInvalid, "a Codex fork must copy at least one binding")
	}
	fields, appErr := normalizeManagedProfileFields(profileID, req.Name, req.Description)
	if appErr != nil {
		return CodexProfileSaveResult{}, appErr
	...
	var operationID string
	err = service.maintenance.RunMaintenance(ctx, maintenance.Request{
		Operation: "codex-profile-fork", ProfileID: profileID, ProviderID: codexconfig.ProviderID, Record: false,
	}, func(ctx context.Context, txStore *store.Store, currentOperationID string) error {
		operationID = currentOperationID
		...
		if !hasProvider {
			return apperror.New(apperror.ProviderNotFound, "Codex provider not found")
		}
		...
		if err != nil {
			return apperror.Wrap(apperror.StoreStatusFailed, "failed to read source Codex profile targets", err)
		}
...
func (service *Service) UpdateProfileConfigSet(ctx context.Context, req UpdateCodexProfileConfigSetRequest) (CodexProfileDetail, error) {
	if err := service.requireAccess(ctx); err != nil {
	...
	configSetID, appErr := validateID(req.ConfigSetID, apperror.CodexInvalid)
	if appErr != nil {
		return CodexProfileDetail{}, appErr
	}
	var operationID string
	err := service.maintenance.RunMaintenance(ctx, maintenance.Request{
		...
		if exists && active.ProfileID == profileID {
			return apperror.New(apperror.CodexInvalid, "active Codex profile config set cannot be changed").WithDetail("profile_id", profileID)
		}
		...
		if err != nil {
			return apperror.Wrap(apperror.StoreStatusFailed, "failed to read Codex profile targets", err)
		}
		...
		if _, err := txStore.UpsertProfileConfigSetBinding(ctx, store.UpsertProfileConfigSetBindingParams{
			...
			return apperror.Wrap(apperror.StoreStatusFailed, "failed to update Codex config binding", err)
		}
		_ = configTarget
...
func (service *Service) SaveActiveProfileStateFor(ctx context.Context, expectedProfileID string, expectedCredentialReferences, expectedConfigReferences int) (CodexProfileStateSaveResult, error) {
	return service.saveActiveProfileState(ctx, expectedProfileID, expectedCredentialReferences, expectedConfigReferences)
}

func (service *Service) saveActiveProfileState(ctx context.Context, expectedProfileID string, expectedCredentialReferences, expectedConfigReferences int) (CodexProfileStateSaveResult, error) {
	if err := service.requireAccess(ctx); err != nil {
	...
	var operationID string
	err = service.maintenance.RunMaintenance(ctx, maintenance.Request{
		Operation: "codex-profile-save-current", ProviderID: codexconfig.ProviderID, Record: false,
	}, func(ctx context.Context, txStore *store.Store, currentOperationID string) error {
		operationID = currentOperationID
		_, hasProvider, err := codexPreflightProvider(ctx, txStore, home)
		if err != nil {
		...
		if expectedProfileID != "" {
			credentialReferences, err := codexCredentialBindingCount(ctx, txStore, credentialID)
			if err != nil {
				return err
			}
			configReferences, err := codexConfigSetBindingCount(ctx, txStore, configSet.ID)
			if err != nil {
				return err
			}
			if credentialReferences != expectedCredentialReferences || configReferences != expectedConfigReferences {
				return apperror.New(apperror.ProfileSharingChanged, "Codex profile sharing changed before save")
			}
		}
...
func resolveCreatedProfileConfigSet(ctx context.Context, db *store.Store, content string, req CreateCodexProfileRequest) (store.ProviderConfigSet, error) {
	...
	if !activeExists && newID != "" {
		return store.ProviderConfigSet{}, apperror.New(apperror.CodexInvalid, "the first Codex profile must create the shared config set")
	}
	if activeExists && newID == "" {
		targets, err := storedCodexBindingTargets(ctx, db, active.ProfileID)
		if err != nil {
			return store.ProviderConfigSet{}, apperror.Wrap(apperror.StoreStatusFailed, "failed to read active Codex profile targets", err)
		}
	...
	id, appErr := validateID(newID, apperror.CodexInvalid)
	if appErr != nil {
	...
	if req.NewConfigSetName != nil {
		name, appErr = validateName(*req.NewConfigSetName, apperror.CodexInvalid)
		if appErr != nil {
			return store.ProviderConfigSet{}, appErr
		}
	}
	...
	if req.NewConfigSetDescription != nil {
		description, appErr = validateDescription(*req.NewConfigSetDescription, apperror.CodexInvalid)
		if appErr != nil {
			return store.ProviderConfigSet{}, appErr
		}
	}
...
func copyForkedCodexConfigSet(ctx context.Context, db *store.Store, source store.ProviderConfigSet, req ForkCodexProfileRequest) (store.ProviderConfigSet, error) {
	id, appErr := validateID(req.NewConfigSetID, apperror.CodexInvalid)
	if appErr != nil {
		return store.ProviderConfigSet{}, appErr
	}
	if _, err := db.GetProviderConfigSet(ctx, codexconfig.ProviderID, id); err == nil {
...
func createCodexProfileTargets(ctx context.Context, db *store.Store, profileID string, home codexconfig.Home, configSetID, credentialID string) (store.ProfileTarget, store.ProfileTarget, error) {
	...
	if targets.HasConfig || targets.HasAuth {
		return store.ProfileTarget{}, store.ProfileTarget{}, apperror.New(apperror.ProfileAlreadyExists, "Codex profile targets already exist").WithDetail("profile_id", profileID)
	}
...
func requireCodexFullProfileTargets(profileID string, targets []store.ProfileTarget) (store.ProfileTarget, store.ProfileTarget, error) {
	return codexprofile.FullProfileTargets(profileID, targets)
}
...
func loadCodexConfigContent(home codexconfig.Home, content *string) (string, error) {
	if content == nil {
		...
		if missing {
			return "", apperror.New(apperror.CodexInvalid, "Codex config.toml is required").WithDetail("config_path", home.ConfigPath)
		}
		return configContent, nil
	}
	if len(*content) > targetfs.MaxFileBytes {
		return "", apperror.New(apperror.CodexInvalid, "Codex config is too large").WithDetail("config_path", home.ConfigPath)
	}
	if err := codexconfig.ValidateTOML(*content); err != nil {
		return "", apperror.New(apperror.CodexInvalid, "Codex config TOML is invalid").WithDetail("config_path", home.ConfigPath)
	}
	return *content, nil
}
...
func normalizeCodexForkBinding(raw, kind string) (string, *apperror.Error) {
	value := strings.TrimSpace(raw)
	if value == CodexForkBindingShareParent || value == CodexForkBindingCopyNew {
		return value, nil
	}
	return "", apperror.New(apperror.CodexInvalid, "unsupported Codex fork "+kind+" binding").
...
func wrapCodexMutationTxError(message string, err error) error {
	...
	if errors.Is(err, os.ErrNotExist) {
		return apperror.New(apperror.CodexInvalid, "Codex profile source is missing")
	}
	return apperror.Wrap(apperror.StoreStatusFailed, message, err)
}
