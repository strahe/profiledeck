...
func (service *Service) ListProfiles(ctx context.Context) (CodexProfileListResult, error) {
...
func (service *Service) GetProfile(ctx context.Context, rawProfileID string) (CodexProfileDetail, error) {
	if err := service.requireAccess(ctx); err != nil {
		return CodexProfileDetail{}, err
	}
	profileID, appErr := validateID(rawProfileID, apperror.ProfileInvalid)
	if appErr != nil {
...
func getCodexProfileFromStore(ctx context.Context, db *store.Store, profileID string) (CodexProfileDetail, error) {
	...
	if err != nil {
		return CodexProfileDetail{}, apperror.Wrap(apperror.StoreStatusFailed, "failed to list Codex profile targets", err)
	}
	if len(targets) == 0 {
		return CodexProfileDetail{}, apperror.New(apperror.ProfileNotFound, "Codex profile not found").WithDetail("profile_id", profileID)
	}
...
func listCodexProfileSummaries(ctx context.Context, db *store.Store) ([]CodexProfileSummary, error) {
	...
	if err != nil {
		return nil, apperror.Wrap(apperror.StoreStatusFailed, "failed to list Codex profile targets", err)
	}
...
func codexActiveState(ctx context.Context, db *store.Store) (store.ActiveState, bool, error) {
	...
	if err != nil {
		return store.ActiveState{}, false, apperror.Wrap(apperror.StoreStatusFailed, "failed to read active Codex profile state", err)
	}
