...
<div class="mx-auto flex w-full max-w-5xl flex-col gap-4">
	<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
		<div class="flex min-w-0 flex-col gap-1">
			...
			<h2 class="text-xl font-semibold tracking-tight">{$_("configSets.title")}</h2>
			<p class="text-sm text-muted-foreground">{$_("configSets.description")}</p>
		</div>
		<Button size="sm" onclick={onCreate} disabled={busy}>
			<PlusIcon data-icon="inline-start" />{$_("configSets.create")}
		</Button>
	</div>
	...
	<Card.Root>
		<Card.Content class="px-0">
			...
			{:else}
				{#each configSets as configSet (configSet.id)}
					<div class="flex items-center gap-4 border-b px-4 py-3 last:border-b-0">
						<div class="flex min-w-0 flex-1 flex-col gap-1">
							<div class="flex flex-wrap items-center gap-2">
								<span class="font-medium">{configSet.name}</span>
								<span class="font-mono text-xs text-muted-foreground">{configSet.id}</span>
								{#if configSet.active}<Badge variant="secondary">{$_("status.active")}</Badge>{/if}
