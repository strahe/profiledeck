<script lang="ts">
	...
	import type { CodexForkBinding, ManagedDetectResult, ManagedProfileDetail } from "./types";

	interface Props {
		mode: "new" | "fork";
		detail?: ManagedProfileDetail | null;
		detectResult: ManagedDetectResult | null;
		basePath: string;
		copyRoot?: "profilePages" | "grokBuild.profilePages";
	...
	let title = $derived(mode === "new" ? $_(`${copyRoot}.new.title`) : $_("profilePages.fork.title", { values: { profile: detail?.summary.profile.name || detail?.summary.profile.id || "" } }));
...
<div class="mx-auto flex w-full max-w-4xl flex-col gap-4">
	...
	<div class="flex flex-col gap-1">
		...
		<p class="text-sm text-muted-foreground">{mode === "new" ? $_(`${copyRoot}.new.description`) : $_("profilePages.fork.description")}</p>
	</div>

	{#if mode === "new"}
		<Card.Root>
			<Card.Header><Card.Title>{$_(`${copyRoot}.source.title`)}</Card.Title><Card.Description>{$_(`${copyRoot}.source.description`)}</Card.Description></Card.Header>
	...
	<Card.Root>
		<Card.Header><Card.Title>{$_("profilePages.configChoice.title")}</Card.Title><Card.Description>{$_(mode === "new" && !canChooseConfigSet ? `${copyRoot}.configChoice.firstDescription` : `${copyRoot}.configChoice.description`)}</Card.Description></Card.Header>
			...
			{#if mode === "new" && canChooseConfigSet}
				<RadioGroup.Root bind:value={configMode} class="grid gap-3 md:grid-cols-2">
					<Field.FieldLabel class="items-start rounded-lg border p-3">
						<RadioGroup.Item value="reuse" /><Field.FieldContent><Field.FieldTitle>{$_("profilePages.configChoice.reuse")}</Field.FieldTitle><Field.FieldDescription>{$_(`${copyRoot}.configChoice.reuseDescription`)}</Field.FieldDescription></Field.FieldContent>
					</Field.FieldLabel>
					<Field.FieldLabel class="items-start rounded-lg border p-3">
						<RadioGroup.Item value="new" /><Field.FieldContent><Field.FieldTitle>{$_("profilePages.configChoice.new")}</Field.FieldTitle><Field.FieldDescription>{$_(`${copyRoot}.configChoice.newDescription`)}</Field.FieldDescription></Field.FieldContent>
					</Field.FieldLabel>
				</RadioGroup.Root>
			{:else if mode === "new"}
				<div class="flex items-center gap-2 text-sm"><Badge variant="outline">shared</Badge><span class="text-muted-foreground">{$_(`${copyRoot}.configChoice.currentSettings`)}</span></div>
			{:else}
		...
		<Card.Footer class="justify-end gap-2">
			...
			<Button disabled={!submitReady || busy} onclick={onSubmit}>{#if busy}<Spinner data-icon="inline-start" />{:else}<SaveIcon data-icon="inline-start" />{/if}{mode === "new" ? createActionLabel || $_("actions.saveAsNewProfile") : $_("actions.fork")}</Button>
		</Card.Footer>
	</Card.Root>
</div>
