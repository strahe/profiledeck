...
恢复可能会还原工具自己的文件或所选系统登录，但不会改变当前 Profile。如果未完成切换后当前 Profile 发生变化、目标被其他程序修改、恢复数据损坏或目标无法读取，ProfileDeck 会拒绝写入并提示需要检查的内容。失败后仍可针对原始切换重试。
...
如果 ProfileDeck 提示本地数据格式不受支持，则无法就地更新这些数据。请恢复一份兼容的应用备份。如果要改用新的本地数据，请关闭所有 ProfileDeck 进程，将整个 [ProfileDeck 数据目录](../reference/data-security.md) 移到私密位置，然后重新打开 ProfileDeck 或运行 `profiledeck-cli init`。移动目录会保留旧数据库和备份供后续检查，不会更改工具自己的文件或系统登录。
