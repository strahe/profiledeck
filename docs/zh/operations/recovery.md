...
如果 ProfileDeck 提示本地数据格式不受支持，则无法就地更新这些数据。请恢复一份兼容的应用备份。如果要改用新的本地数据，请关闭所有 ProfileDeck 进程，将整个 [ProfileDeck 数据目录](../reference/data-security.md) 移到私密位置，然后重新打开 ProfileDeck 或运行 `profiledeck init`。移动目录会保留旧数据库和备份供后续检查，不会更改工具自己的文件或系统登录。
