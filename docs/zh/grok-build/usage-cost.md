...
| `grok-4.6` | $2.00 / 100 万令牌 | $0.50 / 100 万令牌 | $6.00 / 100 万令牌 |
| `grok-4.6-build` | $2.00 / 100 万令牌 | $0.50 / 100 万令牌 | $6.00 / 100 万令牌 |
| `grok-build-latest` | $2.00 / 100 万令牌 | $0.30 / 100 万令牌 | $6.00 / 100 万令牌 |

价格来源为 [xAI 定价](https://docs.x.ai/developers/pricing)。[Grok Build 使用 Grok 4.6](https://docs.x.ai/build/overview)，因此 ProfileDeck 将当前会话记录中的 `grok-4.6-build` 标识按 Grok 4.6 等价价格计算。聚合后的会话记录无法判断单次请求是否进入长上下文价格阶梯，因此 ProfileDeck 不会应用 2 倍的长上下文价格。

如果会话记录包含缓存创建令牌，ProfileDeck 仍会保留该事件，但会报告部分估算，因为当前用量事实只按输入、缓存输入和输出令牌计价。
