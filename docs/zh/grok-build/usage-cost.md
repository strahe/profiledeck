...
你可以安全地重复同步，已导入的用量不会再次计数。缺少用量、空用量或用量不完整的记录会被跳过。如果文件在读取期间发生变化，包含过大或格式错误的终态记录，或出现无法识别的终态格式，ProfileDeck 不会提交该文件中的任何新数据，并会在后续同步时重试。源文件不会被移动或修改。

Fork 复制的历史只计算一次。同一个已完成回合以相同用量出现在多个会话时，ProfileDeck 会将其稳定归入一个派生会话，不保存原始会话标识；如果同一回合的用量冲突，则改为重试受影响的文件。
