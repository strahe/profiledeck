...
ProfileDeck 用于安全切换本地 AI 编程工具的 profile。当前实现以 Codex 为主：可以从用户配置和文件形式的授权信息创建完整 Codex profile，在已保存的 profile 之间切换，导入本地 token 用量，并从中断的切换操作中恢复。
...
- 在完整文件 profile 之间共享或 fork 隐藏 Codex auth credential。
...
- [Codex Profile](/zh/codex/profiles) 说明 Codex 完整 profile 工作流。
